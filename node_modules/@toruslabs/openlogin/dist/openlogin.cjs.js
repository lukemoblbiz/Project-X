...
class OpenLogin {
  constructor(options) {
    ...
    if (options.useMpc && !options.sdkUrl) {
      if (Object.values(openlogin_utils_namespaceObject.TORUS_LEGACY_NETWORK).includes(options.network)) throw new Error("MPC is not supported on legacy networks, please use sapphire_devnet or sapphire_mainnet.");
      if (options.buildEnv === openlogin_utils_namespaceObject.BUILD_ENV.DEVELOPMENT) {
        options.sdkUrl = "http://localhost:3000";
      } else if (options.buildEnv === openlogin_utils_namespaceObject.BUILD_ENV.STAGING) {
